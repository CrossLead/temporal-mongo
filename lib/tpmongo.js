 ...
 *  * Maps to raw `insert()` or `update()` calls and returns corresponding `WriteResult`s
 * @param  {Object} doc document to save
 * @param  {Object} options primarily `writeConcern`
 * @return {WriteResult} metadata if updated, document if inserted (matching promised-mongo API for each op)
 */
TemporalCollection.prototype.save = function(doc, options) {
  ...
  if(doc._rId === undefined) {
    ...
    return _this.findOneRaw({_rId: doc._rId})
      .then(function(foundDoc) {
        if(foundDoc && foundDoc._rId) {
          const update = _.omit(doc, '_id');
          return _this.update({_rId: doc._rId}, {$set: update}, options);
        } else {
