...
TemporalCollection.prototype.save = function(doc, options) {
  ...
  if(doc._id === undefined) {
    ...
    return _this.findOneRaw({_id: doc._id})
    .then(function(foundDoc) {
      if(foundDoc && foundDoc._id) {
        return _this.update({_id: doc._id}, {$set: {_replaceMe: true}})
        .then(function() {
          return _this.findOneRaw({_rId: foundDoc._rId, _current: 1});
        })
        .then(function(findOneResult) {
          doc._id = findOneResult._id;
          doc._rId = findOneResult._rId;
          doc._current = findOneResult._current;
          doc._startDate = findOneResult._startDate;
          doc._endDate = findOneResult._endDate;
          return _this.saveRaw(doc);
        });
      } else {
        //doc not found, insert
...
TemporalCollection.prototype.aggregate = function(pipeline, options) {
  ...
  modifiedPipeline.$match._current = 1;
...
TemporalCollection.prototype.mapReduce = function(map, reduce, options) {
  ...
  modifiedOptions.query._current = 1;
...
TemporalCollection.prototype.insert = function(doc, options) {
  doc._id = doc._id || pmongo.ObjectId();
  doc._rId = doc._id; 
  doc._current = 1;
  ...
  .then(function(sourceDocuments) {
    ...
    if(sourceDocuments.length !== lockedDocumentCount) {
      ...
      .then(function() {
        ...
        _this.addSetterToUpdate(docUpdate, '_current', 1);
...
TemporalCollection.prototype.findAndModify = function(options, attempt) {
  var _this = this;
  var currentDate = _this.getCurrentDate();
  var currentQuery = util._extend({_current: 1}, options.query);
  ...
  return _this.findAndModifyExRaw({query: currentQuery, sort: sort, update: {$set: {_locked: 1}}})
  .then(function(result) {
    if(result.result && result.result._locked === undefined) {
      ...
      .then(function() {
        ...
        return _this.updateRaw(unlockQuery, unlockUpdate)
        .then(function() {
      ...
      .then(function() {
        return _this.findOneRaw({_id: upsertId}, options.fields || {});
      });
    }
    else {
...
TemporalCollection.prototype.remove = function(query, options) {
  ...
  return _this.updateRaw(modifiedQuery, {$set: {_current:0, _endDate: this.getCurrentDate()}}, {multi: multi})
  .then(function(result) {
...
TemporalCollection.prototype.temporalize = function() {
  ...
  .then(function(alreadyTemporalizedCount) {
    if(alreadyTemporalizedCount > 0) {
      ...
      return _this.temporalizeBatch(pmongo.ObjectId('000000000000000000000000'));
    }
  });
};

TemporalCollection.prototype.temporalizeBatch = function(lastId) {
  var _this = this;
  return _this.findRaw({_rId: {$exists: false}, _id: {$gt: lastId}}, {_id: 1}).sort({_id: 1}).limit(10).toArray()
  .then(function(docsToTemporalize) {
    if(docsToTemporalize && docsToTemporalize.length > 0) {
      var updatePromise = Q.when(true);
      for(var docIter = 0; docIter < docsToTemporalize.length; docIter++) {
        ...
        updatePromise = updatePromise.then(function() {
          return _this.findAndModifyExRaw( { query: { _id:  docId}, update: { $set: { _current: 1, _rId: docId, _startDate: startDate, _endDate: _this.config._maxDate } }} );
        });
      }    
      return updatePromise
      .then(function() {
        return _this.temporalizeBatch(docsToTemporalize[docsToTemporalize.length-1]._id);
      });
    } else {
      return true;
    }
...
TemporalCollection.prototype.ensureIndexes = function() {
  ...
  _this.ensureIndex({_current: 1});
  _this.ensureIndex({_transIds: 1});
  _this.ensureIndex({_current: 1, _transIds: 1});
...
TemporalCollection.prototype.cleanLocks = function() {
  ...
  cleaningPromises.push(_this.updateRaw(unlockCurrentDocumentsQuery, {$unset: {_locked: ''}}, {multi: true, writeConcern: 'majority'})
  .then(function() {
