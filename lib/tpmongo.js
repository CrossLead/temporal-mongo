...
TemporalCollection.prototype.temporalize = function() {
  ...
  .then(function() {
    console.log('Temporalize Complete');
  })
  .catch(function(err) {
    console.log('Error Temporalizing');
    console.log(err);
    return Q.reject('Error Temporalizing');
  });
};
  ...
  .then(function(docsToTemporalize) {
    if(docsToTemporalize && docsToTemporalize.length > 0) {
      ...
      for(var docIter = 0; docIter < docsToTemporalize.length; docIter++) {
        ...
        updatePromises.push(_this.findAndModifyExRaw( { query: { _id:  docId}, update: { $set: { _current: 1, _rId: docId, _startDate: startDate, _endDate: _this.config._maxDate } }} ));
      }
