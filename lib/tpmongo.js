  ...
  if(doc._id === undefined) {
    ...
    .then(function(foundDoc) {
      if(foundDoc && foundDoc._id) {
        ...
        .then(function() {
          return _this.findOneRaw({_rId: foundDoc._rId, _current: CurrentStatus.Current});
        })
  ...
  .then(function(result) {
    if(result.result && result.result._locked === undefined) {
      ...
      .then(function() {
        ...
        var unlockUpdate = {$set: {_current: CurrentStatus.Archived, _endDate: currentDate}, $unset: {_locked: ''}};
...
TemporalCollection.prototype.cleanLocks = function() {
  ...
  removeOrphanedCloneQuery._current = CurrentStatus.InProgressClone;
  ...
  unlockCurrentDocumentsQuery._current = CurrentStatus.Current;
