...
var proxyCollection = pmongo.Collection;
var proxyCollectionPrototype = pmongo.Collection.prototype;
...
var TemporalMongo = function(connectionString, collections) {
  ...
  _.forEach(collections, function(collectionName) {
    ...
    db[collectionName] = new TemporalCollection(collectionName);
    db[collectionName].proxyCollection = db.proxyCollections[collectionName];
    db[collectionName].ensureIndexes();
...
TemporalCollection.prototype.findAndModify = function(options, attempt) {
  ...
  //return _this.findAndModifyRaw({query: currentQuery, {$set: {_locked: 1}})
  ...
  .then(function(result) {
    if(result.result && result.result._locked === undefined) {
      ...
      .then(function() {
        ...
        .then(function(updateResult) {
          if(options['new']) {
            return _this.findOneRaw({_id: clone._id}, options.fields || {});
          } else {
            return _this.findOneRaw({_id: result._id}, options.fields || {}); 
          }
          //return Q.when(updateResult);
        }, function(err) {
          return Q.reject(_this.TemporalMongoError('LOCK_RELEASE_FAILED', 
...
TemporalCollection.prototype.ensureIndexes = function() {
  ...
  _this.proxyCollection.ensureIndex({_current: 1});
  _this.proxyCollection.ensureIndex({_transIds: 1});
  _this.proxyCollection.ensureIndex({_current: 1, _transIds: 1});
  _this.proxyCollection.ensureIndex({_current: 1});
  _this.proxyCollection.ensureIndex({_current: 1});
};
