...
/**
 * Determine if an update param (for update() and findAndModify()) is intended
 * to replace the entire document or perform `$set`s and `$unset`s.
 * 
 * @param  {Object} update parameter passed to update() or findAndModify()
 * @return {Boolean} true if intended to replace entire doc
 */
export function isReplaceEntireDocUpdate(update) {
  return Object.keys(update).every(el => {
