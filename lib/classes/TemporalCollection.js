...
export default class TemporalCollection extends PromisedMongoCollection {
  ...
  async update(query, docUpdate, options, transactionId, attempt=0) {
    ...
    const currentDate                   = this.getCurrentDate(),
          ...
          queryForCurrent               = Object.assign({_current: CurrentStatus.Current}, query),
          queryForCurrentByTransaction  = Object.assign({'_tranIds.0': tranId}, queryForCurrent),
          optionsForLocking             = Object.assign({writeConcern: 'majority'}, options || {}),
    ...
    if(sourceDocuments.length !== lockedDocumentCount) {
      ...
      try {
        if(attempt > this.config._retryAttempts) {
          ...
          return this.update.call(this, query, sanitizedDocUpdate, options, tranId, attempt);
        }
      } catch (err) {
        return sendErr(
  ...
  async findAndModify(options, attempt=0) {
    ...
    const currentDate  = this.getCurrentDate(),
          currentQuery = Object.assign({_current: CurrentStatus.Current}, options.query),
