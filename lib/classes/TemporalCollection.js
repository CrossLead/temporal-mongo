...
import PromisedMongoCollection  from 'promised-mongo/dist/lib/Collection';
import CurrentStatus            from '../enum/CurrentStatus';

import {  omit,
          sanitizeUpdate,
          isReplaceEntireDocUpdate,
          delay,
          addDate,
          addCurrent,
...
export default class TemporalCollection extends PromisedMongoCollection {
   ...
   * @param {Boolean} cursor : return cursor instead of promise
   * @return {Promise}
   */
  _aggregateWrapper(restriction, date, pipeline, options, cursor = false) {
    ...
    if (cursor) {
      return this.aggregateCursorRaw(pipeline, options);
    }

    return this.aggregateRaw(pipeline, options);
  }

  aggregate(pipeline, options) {
    return this._aggregateWrapper(addCurrent, null, pipeline, options);
  }

  aggregateCursor(pipeline, options) {
    return this._aggregateWrapper(addCurrent, null, pipeline, options, true);
  }

  aggregateByDate(date, pipeline, options) {
    if (!(date && date instanceof Date)) {
      throw new Error('date required for aggregateByDate, none given!');
    }
    return this._aggregateWrapper(addDate, date, pipeline, options);
  }

  aggregateByDateCursor(date, pipeline, options) {
    if (!(date && date instanceof Date)) {
      throw new Error('date required for aggregateByDate, none given!');
    }
    return this._aggregateWrapper(addDate, date, pipeline, options, true);
  }
  ...
  async update(query, docUpdate, options, transactionId, attempt=0) {
    ...
    if(isReplaceEntireDocUpdate(sanitizedDocUpdate)) {
      return sendErr('UNSUPPORTED_OPERATION', 'Whole document replacement not supported for update.');
    }
  ...
  async findAndModify(options, attempt=0) {
    ...
    try {
      if(result.value && result.value._locked === undefined) {
        ...
        if (isReplaceEntireDocUpdate(docUpdate)) {
          // Ensure protected fields don't get unset
          docUpdate._rId = result.value._rId;
          docUpdate._current = clone._current;
          docUpdate._startDate = clone._startDate;
          docUpdate._endDate = clone._endDate;
        }
        ...
        const wrappedUpdate = isReplaceEntireDocUpdate(docUpdate) ? { $set: docUpdate } : docUpdate;
        await this.updateRaw({_rId: upsertId}, wrappedUpdate);

        // Match findAndModify() result format
        if(options.new) {
          return this.findAndModifyRaw({
            query: {_rId: upsertId},
            update: {$unset: {fieldThatDoesNotExist:''}},
            new: true
          });
        } else {
          return null;
  ...
  addUnsetterToUpdate(update, propertyToUnset) {
    if (isReplaceEntireDocUpdate(update)) {
      delete update[propertyToUnset];
    } else {
  ...
  async temporalize(retemporalize = false) {
    const batchCount = 0,
    ...
    try {
      ...
      if(alreadyTemporalizedCount > 0 && !retemporalize) {
        return Promise.reject('Found temporalized documents in collection with retemporalize=false');
      } else {
        return this.temporalizeBatch(
          pmongo.ObjectId('000000000000000000000000'),
          documentsPerBatch,
          batchCount,
          retemporalize
        );
  ...
  async temporalizeBatch(lastId, documentsPerBatch, batchCount, retemporalize = false, skip = 0) {
    batchCount++;
    ...
    if(docsToTemporalize && docsToTemporalize.length > 0) {
      ...
      await* docsToTemporalize.map(doc => {
        if (!retemporalize && (doc._rId || doc._current || doc._startDate || doc._endDate)) {
          return doc;
        }
